...
public class DataContinuousWriter : MonoBehaviour
{
    ...
    [Header("Default Transforms. Auto-fill if you have a ResXRPlayer")]
    ...
    public void Init(bool ShouldRecordEyeTracking)
    {
        ...
        //setup file
        filePath = Path.Combine(Application.persistentDataPath, $"{fileName}_{ResXRUtilities.GetFormattedDateTime(true)}.csv");
    ...
    {
        // TODO: Move logging frequency to ResXRDataManager
        if (logFrequency == 0 || Time.time - lastLogTime >= logFrequency)
        {
            ...
            if (IsEyeTrackingEnabled)
            {
                if (ResXRPlayer != null)
                {
                    if (ResXRPlayer.FocusedObject != null)
                    {
                        rowBuilder.Append("," + ResXRPlayer.FocusedObject.name);
                    }
                    else
                    {
                    ...
                    rowBuilder.Append("," + ResXRPlayer.EyeGazeHitPosition.x.ToString("F4"));
                    rowBuilder.Append("," + ResXRPlayer.EyeGazeHitPosition.y.ToString("F4"));
                    rowBuilder.Append("," + ResXRPlayer.EyeGazeHitPosition.z.ToString("F4"));

                    rowBuilder.Append("," + ResXRPlayer.RightEye.localEulerAngles.x.ToString("F4"));
                    rowBuilder.Append("," + ResXRPlayer.RightEye.localEulerAngles.y.ToString("F4"));
                    rowBuilder.Append("," + ResXRPlayer.LeftEye.localEulerAngles.x.ToString("F4"));
                    rowBuilder.Append("," + ResXRPlayer.LeftEye.localEulerAngles.y.ToString("F4"));
                }
            }
