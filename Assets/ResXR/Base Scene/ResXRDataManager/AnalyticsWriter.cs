...
// write analytics tables to separate csv files for each analytic type. Used in ResXRDataManager.
public class AnalyticsWriter
{
    ...
    public AnalyticsWriter()
    {
        ...
        //dataPath = Path.Combine(Application.persistentDataPath, "AnalyticsEvents_" + ResXRUtilities.GetFormattedDateTime(true));
    ...
    // called from ResXRDataManager on OnApplicationQuit
    public void Close()
    {
    ...
    // Called from ResXRDataManager every time a new line is logged into a Analytics Table.
    ...
    private void CreateNewDataFile(AnalyticsDataClass dataClass)
    {
        ...
        string csvFilePath = Path.Combine(dataPath, fileName + $"_{ResXRUtilities.GetFormattedDateTime(true)}.csv");
        ...
        List<string> fieldNames = ResXRUtilities.SerializeObject(dataClass).Keys.ToList();
        ...
        {
            Debug.LogError($"Tried to remove TableName from a data class but couldn't find a member with TableName name. Look at your Analytics Data classes under ResXRDataManager and make sure they all have a string member called TableName");
        }
    ...
    private void WriteLineInFile(AnalyticsDataClass dataClass)
    {
        ...
        Dictionary<string, string> lineData = ResXRUtilities.SerializeObject(dataClass);
